 ...
 const gridView = new GridView({ 
        container:document.getElementById("grid-container"),
         ...
         onSheetSelected : (page) => { 
                     generateAllTraitSheet( page, 0 );
             } 
         }
     );
      const sheetView = new SheetView(
             document.getElementById("sheetPreview")
      );
...
const layoutManager = new LayoutManager();
 ...
 layoutManager.registerSlot( "sheet",document.getElementById("sheetTraitSlot"));
...
//============================   QUERY BOX =======================================
 ...
 async function refreshQueryResult (raw) {
           ...
           if (  result && result.queryMode === "NFT_SEARCH" ){
             ...
             if ( result && result.queryMode === 'DSL'){ 
                 ...
                 const errors = result.queryResult?.errors ;
...
//=====================================================
 ...
 //===========================
  ...
  const prevBatchButton = new RunButton({
    ...
    prevBatchButton.button.classList.add("btn_navSheet");
...
//======================================================== 
 ...
 const nextBatchButton = new RunButton({
    ...
    nextBatchButton.button.classList.add("btn_navSheet");
...
const filterModeToggle = new ToggleButton({
    ...
    containerId: "trait-pill-container",//"sheetTraitSlot",// "trait-pill-container",// "buttonSet2",
     id:"toggleButton",
     label: "Filter",
...
// make sure all variables are up to date after api response/result
  function update_activeFilterMap(result){ 
          //============================== Client UI display ==============================  
               // uiRegistry.searchTypeResult.textContent = result.queryMode;              
               // uiRegistry.searchTypeCount.textContent = result?.activeFilterMap_IDS.length;
           ...
           // update grid IDS result for dislpay
                ...
                console.log( "== > update  get_UIstate()" ,   get_UIstate());

                 
                 viewManager.show("SEARCH RESULT");
               generateAllTraitSheet( null ,0 );
